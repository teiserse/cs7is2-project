...
We decided on search as an example of an unsuitable approach, Q-Learning \todo[inline]{ask andrej}, constraint satisfaction because of the natural mapping between the puzzle to the approach, and Genetic Algorithms, due to the dissimilarity of the approach from the other ones we were attempting.
...
The two criteria the algorithms were judged on were solve time and solve rate. The algorithm that best fit these criteria is the CSP algorithm, without compare.
Not only did it solve every single nonogram, it also achieved to lowest average times.
...
The reason for this is due to the nature of the problem structure. Picross problems requires a solver to mechanically consider many different options available and choosing them in a smart way and paying close attention to the constraints will allow the problem and search space to be reduced.
...
In most cases, it is not practical to judge how far away from a solution a particular state is. As a result, creating a heuristic for the problem is incredibly difficult. A uniform cost approach does not consider problem reduction like the constraint search approach and as a result, the performance is lacking. As a result, it leads to an extremely large state space without providing acceptable results.
